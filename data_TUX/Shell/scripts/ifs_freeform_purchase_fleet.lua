...
ifs_freeform_purchase_fleet = NewIFShellScreen {
	...
	UpdateAction = function(this)
		...
		if ifs_freeform_main.planetTeam[selected] ~= team then
			...
			IFText_fnSetString(this.description.caption, "ifs.freeform.purchase.navy.nobuild")
			IFObj_fnSetColor(this.description.caption, 255, 255, 255)
		...
		elseif ifs_freeform_main.planetPort[selected] ~= team then
			-- build a port
			IFText_fnSetString(this.description.caption, "ifs.freeform.purchase.navy.portname")
			...
			if ifs_freeform_main:EnoughResources(team, this.portCost) then
				...
				IFObj_fnSetColor(this.description.caption, 255, 255, 32)
				IFObj_fnSetColor(this.description.subcaption, 255, 255, 32)
				this.buildPort = true
			else
				IFObj_fnSetAlpha(this.action.accept, 0.5)
				IFObj_fnSetColor(this.description.caption, 255, 32, 32)
				IFObj_fnSetColor(this.description.subcaption, 255, 32, 32)
			end
