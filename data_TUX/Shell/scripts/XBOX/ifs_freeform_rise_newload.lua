...
ifs_freeform_rise_newload = NewIFShellScreen {
	...
	Enter = function(this, bFwd)
		...
		if(ScriptCB_GetSPProgress(2) > 0) then
			--this.buttons.campaign.hidden = false
			this.buttons.campaign.bDimmed = 0
		else
			--if rise has not yet been beaten, we hide the campaign mission archive list
			--this.buttons.campaign.hidden = true
			this.buttons.campaign.bDimmed = 1
		end
