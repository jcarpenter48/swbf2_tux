...
--				DrawParticleAt(matrix, "planetgraphic_cursor", 16, r, g, b, 192, 0.0)
			...
			if team ~= 0 then
				DrawParticleAt(matrix, "star_flare", 16, r, g, b, 128 * alpha, 0.0)
				DrawParticleAt(matrix, "star_flare", 8, 255, 255, 255, 255 * alpha, 0.0)
			end
		end
	end,
...
--	end,
	...
	SaveState = function(this)
		...
		ScriptCB_SaveMetagameState(
			this.custom,
			this.scenario,      --this.teamCode[1], why would we store that instead of the variable specifically for scenario
...
--		this.planetPort,
		...
		-- perform appropriate scenario
		if this.custom then
			ifs_freeform_start_custom(this, this.custom)
		elseif scenario == "all" then
			ifs_freeform_start_all(this)
		elseif scenario == "imp" then
			ifs_freeform_start_imp(this)
		elseif scenario == "rep" then
			ifs_freeform_start_rep(this)
		elseif scenario == "cis" then
			ifs_freeform_start_cis(this)
		elseif scenario == "cw" then
			ifs_freeform_start_cw(this)		
		elseif scenario == "gcw" then
			ifs_freeform_start_gcw(this)			
		elseif scenario == "ror" then
			ifs_freeform_start_ror(this)	
		else
			assert("undefined scenario type \""..scenario.."\"")
		end
	...
	OneTimeInit = function(this, showLoadDisplay)
		...
		if not this.planetDestination then
			...
			SetMemoryPoolSize("ParticleTransformer::PositionTr", 800)
			SetMemoryPoolSize("ParticleTransformer::SizeTransf", 851)
			SetMemoryPoolSize("ParticleTransformer::ColorTrans", 1176)
			
			SetMemoryPoolSize("ParticleEmitterObject", 24)
...
--			alpha = 1.0,
	...
	local info_w = w * widescreen * 0.98
	local info_h = h * 0.24
	
	this.info = NewIFContainer{
		ScreenRelativeX = 0.012, -- almost left
		ScreenRelativeY = 1.0, --bottom
		width = info_w,
		height = info_h,
		header = "Info",
	...
	local description_h = h * .29
	local description_w = w * widescreen * 0.98
	
	this.description = NewIFContainer {
		ScreenRelativeX = 0.012, -- almost left
		ScreenRelativeY = 1.0, -- bottom
		width = description_w,
		height = h * 0.25,
	...
	if(gPlatformStr == "PC") then
		this.action = NewIFContainer{
			...
			ZPos = 190,
				...
				misc = NewClickableIFButton -- NewRoundIFButton				
				{
					x = w - 60,
					y = -action_h,
				...
				accept = NewClickableIFButton -- NewRoundIFButton			
				{
					x = w - 60,
					btnw = 150, -- made wider to fix 9173 - NM 8/25/04
					btnh = 10,
				...
				back = NewClickableIFButton -- NewRoundIFButton
				{
					x = 60,
					btnw = 150, -- made wider to fix 9173 - NM 8/25/04
					btnh = 10,
		...
		this.action.misc.label.halign = "right"
		this.action.accept.label.halign = "right"
		this.action.back.label.halign = "left"
		RoundIFButtonLabel_fnSetString( this.action.accept, "ifs.mp.create.add" )
		RoundIFButtonLabel_fnSetString( this.action.back,	"common.back" )
		RoundIFButtonLabel_fnSetString( this.action.misc,	"common.next" )		
	else		
	this.action = NewIFContainer{
		ScreenRelativeX = 0.0, -- left
		ScreenRelativeY = 1.0, -- bottom
