...
--		
--		-- update port cost
--		ifs_freeform_purchase_fleet:UpdatePortCost()
--		
--		-- if there is enough resources to build a port...	
--		local funds = ifs_freeform_main.teamResources[myteam]
--		local cost = ifs_freeform_purchase_fleet.portCost
...
--				ifs_freeform_purchase_fleet:BuildPort(myteam, selected)
...
--	end,
	...
	CalculateBuildFleet = function(this, fDt)
		...
		-- update fleet cost
		ifs_freeform_purchase_fleet:UpdateFleetCost()
		
		-- if there is enough resources to build a fleet...	
		local funds = ifs_freeform_main.teamResources[myteam]
		local cost = ifs_freeform_purchase_fleet.fleetCost
		if not cost then cost = 0 end
		print("funds:", funds, "fleet cost:", cost)
		--inserting stuff here to try to fix AI
		-- count fleets
		local fleetsCount = 0
		for planet, fleet in pairs(ifs_freeform_main.planetFleet) do
			if fleet == ifs_freeform_main.playerTeam then
				fleetsCount = fleetsCount + 1
			end
		end		
		if fleetsCount < 1 then
			local planetToBuildFleetOn = nil
			for planet, team in pairs(ifs_freeform_main.planetTeam) do
				if team == myteam and not ifs_freeform_main.planetFleet[planet] then
					planetToBuildFleetOn = planet --get a planet, don't care
				end
			end
			if planetToBuildFleetOn then --we got a planet? good
				-- go to update build fleet
				ifs_freeform_main:SelectPlanet(nil, planetToBuildFleetOn)
				this.displayTime = 2.0
				this.Update = this.UpdateBuildFleet			
			end
		end
		--back to Pandemic
		if funds >= cost and fleetsCount > 0 then
		--if cost and funds >= cost then
			...
			-- pick a location
			--local scaledWeight = totalWeight * funds / (funds - cost * 0.75)	--+ (cost - ifs_freeform_fleet_cost[0]) * cost / funds
	...
	UpdateBuildFleet = function(this, fDt)
		...
		if this.displayTime <= 0 then
			...
			if ifs_freeform_main.planetFleet[selected] then
				...
				ifs_freeform_purchase_fleet:BuildFleet(myteam, selected)
				this.displayTime = 1.0
			end
		end
