...
--	yTop = -70,
	...
	buttonlist = { 
		...
		{ tag = "5", string = "ifs.meta.Configs.ror", },
...
ifs_sp_campaign = NewIFShellScreen {
	...
	bNohelptext_backPC = 1, -- no show back button 
...
--		end
	...
	Input_KeyDown  = function(this, iKey)
		-- -205 ==> Right 
		if(iKey == -205 and gPlatformStr == "PC") then
			-- select instant action screen
			print("Select instant action")
			ScriptCB_PushScreen("ifs_missionselect")
			return
		end
		gInput_KeyDown(this, iKey)
	...
	Input_Accept = function(this)
		...
		elseif (this.CurButton == "ia") then
			ScreenToPush = ifs_missionselect
		...
		elseif (this.CurButton == "load") then
			...
			if(ifs_sp_campaign_fnAskTraining(this)) then 
				...
				elseif (this.CurButton == "5") then
					-- reconquest of the rim scenario
					ifs_freeform_start_ror(ifs_freeform_main)	
				else
					ScreenToPush = nil
				end
			end
