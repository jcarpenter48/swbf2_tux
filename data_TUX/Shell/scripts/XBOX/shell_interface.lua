...
--------------------------------------------------------------------
-- define 'ShellSoundAndMovieSetup()' here in order to give modder the chance to override it
...
if( ScriptCB_IsFileExist("debug\\debug.lvl") == 1) then 
    ReadDataFile("debug\\debug.lvl")
...
--new freeform screens
ScriptCB_DoFile("ifs_freeform_init_ror")
...
--
ScriptCB_DoFile("ifs_freeform_start_ror")
--
...
function ProcessAddons()
    ...
    --for i=0,999,1 do 
    while i < 1000 do 
        addonFile = string.format("addon\\%03d\\addon%03d.lvl", i, i)
        script_name = string.format("addon%03d", i)
		if( ScriptCB_IsFileExist(addonFile) == 1) then 
			print("processing: " .. addonFile)
