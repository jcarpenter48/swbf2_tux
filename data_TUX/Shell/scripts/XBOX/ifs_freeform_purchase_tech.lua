...
ifs_purchase_tech_use_x = -2.5 * ifs_purchase_tech_use_spacing
...
ifs_purchase_tech_table_freeform = {
	...
	--{
	--	mesh = "gal_shell_deathstar",
	--	name = "deathstar",
	--	cost = { [false] = 120, [true] = 100 },
	--	bonus = "leader",
	--	hints = {
			-- death star
	--		{ "^spa", 3 },
...
--			this.main:PlayVoice(string.format(ifs_purchase_tech_name_sound[owned], this.main.playerSide, owned and bonus or name))
			...
			IFText_fnSetString(this.description.caption, string.format(ifs_purchase_tech_localize[owned], name))
			IFText_fnSetString(this.info.text, string.format(ifs_purchase_tech_info_localize[owned], name))
			IFText_fnSetUString(this.description.subcaption, ScriptCB_usprintf("ifs.freeform.credits", ScriptCB_tounicode(cost)))
			local enough = this.main:EnoughResources(nil, cost)
			local a = enough and 1.0 or 0.3
			local r = enough and gTitleTextColor[1] or 165
			local g = enough and gTitleTextColor[2] or 60
			local b = enough and gTitleTextColor[3] or 60
			local a1 = enough and gTitleTextAlpha or 1.0 
			IFObj_fnSetAlpha(this.action.accept, a)
			IFObj_fnSetAlpha(this.description.caption, a1)             -- and I don't mean the sauce!
			IFObj_fnSetAlpha(this.description.subcaption, a1)          -- (but here I do)
			IFObj_fnSetColor(this.description.caption, r, g, b)
			IFObj_fnSetColor(this.description.subcaption, r, g, b)
	...
	UpdateActionUsing = function(this)
			...
			IFText_fnSetString(this.description.caption, string.format(ifs_purchase_tech_localize[true], name))
			IFText_fnSetString(this.info.text, string.format(ifs_purchase_tech_info_localize[true], name))
			IFText_fnSetString(this.description.subcaption, "ifs.freeform.purchase.tech.apply")
...
--						ScriptCB_SetIFScreen(this.miscScreen)
					...
					local ScreenW,ScreenH = ScriptCB_GetSafeScreenInfo()
					if( this.bDoubleClicked == 1 ) then
						this.bDoubleClicked = 0
						if( ( this.iMouse_x > (ScreenW * 0.47) ) and ( this.iMouse_x < (ScreenW * 0.63) ) and
							( this.iMouse_y > (ScreenH * 0.47) ) and ( this.iMouse_y < (ScreenH * 0.82) ) ) then
							-- if click on the card
							print( "this DoubleClicked!" )
						else
							-- do nothing if not click on the card
							return	
						end						
					else
						--print( "mouse x,y = ", this.iMouse_x, this.iMouse_y )
						-- move card if single click
						this.bSingleClicked = 1
						if( this.bSingleClicked ) then
							this.bSingleClicked = nil							
							-- move card in focus mode
							if ( this.focus == ifs_purchase_tech_focus_cards ) then
								if( ( this.iMouse_x < (ScreenW * 0.45) ) ) then
									this:Input_GeneralLeft()
								elseif( ( this.iMouse_x > (ScreenW * 0.65) ) ) then
...
function ifs_purchase_tech_build_screen()
	...
	ifs_freeform_AddCommonElements(ifs_freeform_purchase_tech)
	ifs_freeform_AddTabElements(ifs_freeform_purchase_tech)
