...
function ScriptInit()
    ...
    SetupTeams{
        ...
        gungan = {
            ...
            units = 23,
            reinforcements = -1,
            soldier  = { "gar_inf_soldier",9},
            assault  = { "gar_inf_vanguard",4},
            engineer  = { "gar_inf_pilot",4},
			sniper  = { "gun_inf_soldier",3},
            officer = { "gun_inf_defender",3},
        },
        cis = {
            team = CIS,
            units = 23,
            reinforcements = -1,
            soldier  = { "imp_inf_dark_trooper_hunt",15},
			engineer  = { "imp_inf_engineer",8},
        }
     }
    SetTeamName(1, "Empire")
	SetTeamName(2, "locals")
	
	SetHeroClass(1, "imp_hero_emperor")
    SetHeroClass(2, "gar_inf_naboo_queen")
    --  Level Stats
    ClearWalkers()
    AddWalkerType(0, 1) -- 8 droidekas with 0 leg pairs each
    ...
    SetAmbientMusic(REP, 1.0, "all_nab_amb_hunt",  0,1)
    -- SetAmbientMusic(REP, 0.9, "rep_nab_amb_middle", 1,1)
    -- SetAmbientMusic(REP, 0.1, "rep_nab_amb_end",    2,1)
    SetAmbientMusic(CIS, 1.0, "imp_nab_amb_hunt",  0,1)
    -- SetAmbientMusic(CIS, 0.9, "cis_nab_amb_middle", 1,1)
    -- SetAmbientMusic(CIS, 0.1, "cis_nab_amb_end",    2,1)

    SetVictoryMusic(REP, "all_nab_amb_victory")
    SetDefeatMusic (REP, "all_nab_amb_defeat")
    SetVictoryMusic(CIS, "imp_nab_amb_victory")
    SetDefeatMusic (CIS, "imp_nab_amb_defeat")
